...
namespace PasswordManager.Bot.Commands {
	...
	public class SetUpPasswordGeneratorCommand : Abstractions.BotCommand, IMessageCommand, IActionCommand, ICallbackQueryCommand {
		...
		async Task IActionCommand.ExecuteAsync(Message message, BotUser user) {
			if (user.Action != UserAction.SetUpPasswordGeneratorLength) {
				...
				try {
					...
					if(length > Account.MaxPasswordLength) {
						await Bot.Client.SendTextMessageAsync(message.From.Id,
							"⛓️ " + String.Format(Localization.GetMessage("EnterLength", user.Lang), Account.MaxPasswordLength));
						return;
					}
					...
					await userService.UpdatePasswordGeneratorPattern(user.Id, passwordGeneratorPattern);
					await userService.UpdateActionAsync(user.Id, UserAction.Search);
					await SendGeneratorSettings(user, message);
				}
				catch(Exception) {
					await Bot.Client.SendTextMessageAsync(message.From.Id,
						"⛓️ " + String.Format(Localization.GetMessage("WrongLength", user.Lang), Account.MaxPasswordLength));
				}
			}
		...
		private async Task<string> GetMessageText(BotUser botUser, string passwordGeneratorPattern) {
			...
			try {
				...
				// Default password generator pattern will be set to botUser and a password will be generated by it
		...
		async Task ICallbackQueryCommand.ExecuteAsync(CallbackQuery callbackQuery, BotUser user) {
			...
			switch ((SetUpPasswordGeneratorCommandCode)callbackQuery.Data[1]) {
				...
				case SetUpPasswordGeneratorCommandCode.Length:
					await Bot.Client.EditMessageTextAsync(callbackQuery.Message.Chat.Id, callbackQuery.Message.MessageId,
						"⛓️ " + String.Format(Localization.GetMessage("EnterLength", user.Lang), Account.MaxPasswordLength));
					await userService.UpdateActionAsync(user.Id, UserAction.SetUpPasswordGeneratorLength);
				return;
			}
			...
			//todo move this to botUi
			string messageText = await GetMessageText(user, passwordGeneratorPattern);
			var keyboard = botUi.GetPasswordGeneratorSettingsKeyboard(user, passwordGeneratorPattern);
		...
		async Task IMessageCommand.ExecuteAsync(Message message, BotUser user) => await SendGeneratorSettings(user, message);
